    ...
    const newLayers = layers.map((l: CanvasLayer) => {
      ...
      if (dragState.current.isResizing) {
        ...
        if (settings.snapToGrid) {
          ...
          // When aspect ratio is locked, we need to choose one snap and adjust both dimensions
          if (settings.keepAspectRatio && (snapDx !== 0 || snapDy !== 0)) {
            const aspectRatio = init.width / init.height;

            // Determine which snap to prioritize (the one with smaller delta)
            const absSnapDx = Math.abs(snapDx);
            const absSnapDy = Math.abs(snapDy);

            // If both snaps are active, choose the closer one
            // If only one is active, use that one
            const useXSnap = snapDx !== 0 && (snapDy === 0 || absSnapDx <= absSnapDy);

            if (useXSnap) {
              // Snap to X, adjust height proportionally
              if (activeRight) {
                proposed.width += snapDx;
                const newHeight = proposed.width / aspectRatio;
                if (activeTop) {
                  proposed.y -= (newHeight - proposed.height);
                }
                proposed.height = newHeight;
              } else if (activeLeft) {
                proposed.x += snapDx;
                proposed.width -= snapDx;
                const newHeight = proposed.width / aspectRatio;
              ...
              // Snap to Y, adjust width proportionally
              if (activeBottom) {
                proposed.height += snapDy;
                const newWidth = proposed.height * aspectRatio;
                if (activeLeft) {
                  proposed.x -= (newWidth - proposed.width);
                }
                proposed.width = newWidth;
              } else if (activeTop) {
                proposed.y += snapDy;
                proposed.height -= snapDy;
                const newWidth = proposed.height * aspectRatio;
            ...
            // No aspect ratio lock, apply snaps independently
